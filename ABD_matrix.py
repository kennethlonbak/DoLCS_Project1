...
def test_function():
    ...
    fiber_layup = fib2ABD(fiber_layup)

    ABD_vec, name_ABD = extract_ABC_vec(fiber_layup["ABD"])

    # Get Baseline data
    ABD_baseline = get_baseline_ABD()

    ABD_base_vec= extract_ABC_vec(ABD_baseline)[0]

    fig, ax = py.subplots(1,1,figsize=(6,3))
    #py.rc('text', usetex=True)
    ax.semilogy(range(len(ABD_base_vec)),ABD_base_vec,'.',ms=10.0,label="Baseline")
    ax.semilogy(range(len(ABD_vec)),ABD_vec,'.',label="Own implementation")
    ax.set_xticks(range(len(ABD_base_vec)))
    ax.set_xticklabels(name_ABD, rotation=90)
    ax.grid("on")
    ax.set_xlabel("Matrix entrance")
